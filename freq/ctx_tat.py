...
EVERY = 1              # spread the sample over all 41.6M lines, not one era of cinema
LOG = open(f"{D}/ctx_tat.log", "a", buffering=1)
...
def sentences():
    with open(f"{D}/tat_sentences.txt", encoding="utf-8", errors="replace") as f:
...
def main():
    ...
    for (w, p, i), c in pair.items():
        bytot[(w, p)] += c
    with open(f"{D}/tat_forms.tsv", "w", encoding="utf-8") as f:
        f.write(f"#tokens\t{toks}\n")
        for (w, p, i), c in pair.most_common():
            if bytot[(w, p)] >= 2:
                f.write(f"{w}\t{p}\t{i}\t{c}\n")
    log("wrote tat_forms.tsv")
